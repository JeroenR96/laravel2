...
<body>
	<div class="welcome">
		...
		<h1 class="highlight">Blade have arrived in {{ $theLocation }} on {{ date('Y m d, H i s') }}.</h1>
		@if($theWeather == 'Winderig')
			<p>Het is een Winderige dag</p>
		@elseif($theWeather == 'Zonnig')
			<p>Neem je zonnebril mee</p>
		@else
			<p>Geen weersvoorspelling beschikbaar</p>
		@endif
		<p>Dit zijn develepor codes:</p>
		<ul style="text-align:left">
			@foreach($theCodes as $theCode)
				@unless($theCode == 'HTML')
				<li>{{ $theCode }}</li>
				@endunless
			@endforeach
